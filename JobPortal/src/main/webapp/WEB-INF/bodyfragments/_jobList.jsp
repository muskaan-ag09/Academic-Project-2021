		...
		<tbody>
			<c:forEach items="${list}" var="jb" varStatus="job">
				...
				<%if(uDto.getRoleId()==2){%>
					...
					<%if(uDto.getRoleId()==2){%>
					<td><a href="${editUrl}${jb.id}" class="btn btn-dark">Edit</a>
					</td>
					<%}else{ %>
					<td><a href="${applyUrl}${jb.id}" class="btn btn-dark">Apply</a>
					
						<a href="${awUrl}${jb.id}" class="btn btn-dark">Apply Without Resume</a></td>
					<%} %>
				</tr>
			</c:forEach>
		...
		<div class="col">
			<div class="input-group">
				<input type="submit" name="operation"
					...
					class="btn btn-dark" value="Previous">
			</div>
		</div>
		<%if(uDto.getRoleId()==2){%>
		<div class="col">
